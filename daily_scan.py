...
方式(リプレイ方式):
- 初回実行日を「フォワード開始日」として forward/state.json に固定記録する。
...
FORWARD_DIR = "forward"
STATE_PATH = os.path.join(FORWARD_DIR, "state.json")
TRADES_PATH = os.path.join(FORWARD_DIR, "forward_trades.csv")
...
def load_state() -> dict:
    ...
    print(f"フォワードテスト開始を登録: {state['forward_start']}")
    return state
...
def topix_same_period(data: dict, univ: Universe, cfg: Config, forward_start: pd.Timestamp) -> float:
...
def milestone_review(n: int, data: dict, cfg: Config, forward_start: pd.Timestamp,
                     closed: list, res, topix_ret: float, path: str):
    ...
    lines = [
        ...
        "## 解釈上の注意",
        "",
        "- バックテスト(Phase 1)は期待値 +0.004R の紙一重の合格だった。フォワードで",
        "  期待値がマイナスに沈む・ランダム分布の95%ileを下回るようであれば、Phase 1 の",
        "  結果は選択バイアス・過剰適合の産物だった可能性が高い。",
        "- 判定基準の事後変更は禁止(§10)。",
        "",
    ]
    with open(path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  レビューレポート生成: {path}")


def weekly_summary(closed: list, open_pos: list, res, cfg: Config,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"=== 日次スキャン {datetime.now().strftime('%Y-%m-%d %H:%M')} "
          f"(フォワード開始: {forward_start.date()}) ===")
    data = load_data(cfg, refresh=not args.no_refresh)

    univ, res, closed, open_pos = forward_result(data, cfg, forward_start)
    write_forward_csv(closed, TRADES_PATH)
    topix_ret = topix_same_period(data, univ, cfg, forward_start)
    ...
    print(f"  決済済みトレード: {len(closed)} 件 / 決済後資産: {equity_now:,.0f} 円 "
          f"({equity_now / cfg.initial_capital - 1:+.1%}) / 同期間TOPIX: {topix_ret:+.1%}")
    print(f"  保有中: {len(open_pos)} ポジション"
          + (" — " + ", ".join(t.ticker for t in open_pos) if open_pos else ""))
    if todays:
        print(f"  本日のシグナル(翌営業日寄付でエントリー候補・倍率順):")
    ...
    # 週次サマリー(金曜または --weekly)
    if args.weekly or datetime.now().weekday() == 4:
        weekly_summary(closed, open_pos, res, cfg, forward_start, topix_ret,
                       os.path.join(FORWARD_DIR, "summary_weekly.md"))

    # 30/50 トレード到達レビュー(一度生成したら再生成しない)
    for n in REVIEW_MILESTONES:
        path = os.path.join(FORWARD_DIR, f"review_{n}.md")
        if len(closed) >= n and not os.path.exists(path):
            milestone_review(n, data, cfg, forward_start, closed, res, topix_ret, path)
